...
export default function Location({ config }: { config: LocationConfig }) {
  const items = [
    ...
    { key: '운영', strong: config.hoursWeekday, sub: config.hoursWeekend },
    { key: '교통', strong: config.transit, sub: config.parking },
    {
      key: '연락',
      strong: config.phone,
      sub: [config.kakao && `카카오채널 ${config.kakao}`, config.email].filter(Boolean).join(' · ') || undefined,
    },
  ];
